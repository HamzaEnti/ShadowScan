...
public class MainFrame extends JFrame {
  ...
  // --- PESTANYA 1: DISCOVERY (OSCAR/NICO) ---
  ...
  private void initPantalla() {
    ...
    // Volem fer servir el menú lateral perquè queda més professional, però
    // mantenint l'estructura de TabbedPane que demanava l'exercici.
  ...
  // --- PESTANYA 1: DISCOVERY (Recuperat Fase 1/ i modificat per Nico) ---
  private JPanel createDiscoveryPanel() {
    ...
    // Part modificada per Nico/ es la implementacio dels botons per a fre full
    // escaneig o parcial
    ...
    grpPorts.add(rbPortsComuns);
    grpPorts.add(rbTotsPorts);

    JPanel pnlPorts = new JPanel(new FlowLayout(FlowLayout.LEFT));
    pnlPorts.setBorder(
        BorderFactory.createTitledBorder("Mode d'escaneig de ports"));
    pnlPorts.add(rbPortsComuns);
    pnlPorts.add(rbTotsPorts);
    ...
    // acaba part modificada per Nico
    ...
    btnStart.addActionListener(e -> {
      ...
      PortScanMode mode =
          rbPortsComuns.isSelected() ? PortScanMode.PARCIAL : PortScanMode.FULL;

      System.out.println(
          ">>> [SCAN] Xarxa: " + xarxa + "0/24 | Mode ports: " + mode);
    ...
    btnExport.addActionListener(
        e -> JsonExporter.saveToJSON(resultats, "resultats.json"));
  ...
  private JPanel createNmapPanel() {
    ...
    btnRunNmap.addActionListener(e -> {
      ...
      // Llancem l'escaneig en un fil (Thread) independent. És vital fer-ho
      // així, si ho executéssim al fil principal, la interfície gràfica es
      // quedaria congelada ("No respon").
      new Thread(() -> {
        System.out.println(
            "\n>>> [NMAP] Iniciant anàlisi de serveis a " + ip + " ---");
        nmapService.escanearConNmap(
            ip); // Això es veurà a la consola gràcies al Nico
  ...
  private JPanel createAttackPanel() {
    ...
    btnLlançarHydra.addActionListener(e -> {
      ...
      new Thread(()
                     -> bruteForceService.atacar(txtBruteIp.getText(),
                         Integer.parseInt(txtBrutePort.getText()),
                         fUsers.getAbsolutePath(), fPass.getAbsolutePath()))
          .start();
    });
    ...
    btnLlançarFuzz.addActionListener(e -> {
      ...
      new Thread(()
                     -> fuzzingService.lanzarFuzzing(txtFuzzIp.getText(),
                         Integer.parseInt(txtFuzzPort.getText()),
                         fWordlist.getAbsolutePath()))
          .start();
    });
  ...
  private void initConsole() {
    ...
    scrollConsole.setBorder(BorderFactory.createTitledBorder(
        "Logs del Sistema (Sortida Standard)"));

    this.add(scrollConsole, BorderLayout.SOUTH);
  }

  private void configurarLogs() {
    // Redirigim tot el que surt per consola (System.out) cap al quadre de text
    // de l'app. Així l'usuari pot veure el progrés de les eines en temps real.
  ...
  private void checkDependencies() {
    ...
    if (!nmapService.checkNmapInstalled()) {
      System.err.println(">>> [ERROR] No s'ha detectat Nmap, el botò fallará.");
    } else {
  ...
  public synchronized void afegirResultat(ResultatHost host) {
    ...
    modelTaula.addRow(new Object[] {
        host.getIp(), "[ONLINE]", host.getPortsOberts().toString()});
  }
}